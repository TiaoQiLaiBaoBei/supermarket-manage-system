...
<template>
  <div class="particle">
    <div class="input-wrap">
      <el-input v-model.trim="input" size="small" placeholder="请输入内容" style="margin-right: 10px;"></el-input>
...
export default {
  ...
  methods: {
    ...
    /**
     * 改变显示文字的触发
     * @param this.input 输入框绑定属性
     * @param this.points 所有粒子的集合
     */
    changeShow() {
    ...
    shape() {
      ...
      const data32 = new Uint32Array(data.data.buffer) // 转成32位的数据数组
    ...
    flyAway() {
      ...
      for (let i = 0; i < this.oldPoints.length; i += 1) {
        if (this.oldPoints[i].r === 0) {
          this.oldPoints.splice((i -= 1), 1)
        }
      }
    }
  },
